...
- POST `/api/notes` → 201, `{id:'new-1', text:'Тестовая заметка v24', ...}`
...
**Ожидаемый результат:**
- Текст «Тестовая заметка v24» отображается
- Счётчик обновляется до «Всего заметок: 1»
- POST-запрос содержит заголовок `Authorization: Bearer test-token-v24`
...
- DELETE-запрос содержит заголовок `Authorization: Bearer test-token-v24`
...
2. Инициализировать `waitForResponse` ДО `page.goto()` (AD7)
3. Перейти на `/notes`
