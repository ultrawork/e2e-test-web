...
test.describe('Web Notes Auth v25 — верификация api.ts и /notes', () => {
  ...
  test('SC-003: Создание заметки — счётчик растёт + Authorization в POST', async ({ page }) => {
    ...
    await page.locator('#new-note').fill('Тестовая заметка v25');
    await page.getByRole('button', { name: 'Add' }).click();

    await expect(page.getByText('Тестовая заметка v25')).toBeVisible();
    await expect(page.getByText('Всего заметок: 1')).toBeVisible();
