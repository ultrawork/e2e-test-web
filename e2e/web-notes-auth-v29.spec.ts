...
const SEED_NOTES: Note[] = [
  { id: 1, title: 'Note A v29' },
  { id: 2, title: 'Note B v29' },
];

test.describe('Web v29: /notes Authorization', () => {
  ...
  test('SC-002: GET /api/notes with Bearer token renders notes list', async ({ page }) => {
    ...
    await expect(page.getByText('Note A v29')).toBeVisible();
    await expect(page.getByText('Note B v29')).toBeVisible();
  ...
  test('SC-003: POST /api/notes stateful mock with Authorization Bearer', async ({ page }) => {
    ...
    const notes: Note[] = [{ id: 1, title: 'Existing v29' }];
    ...
    await page.goto('/notes');
    await page.getByPlaceholder('Enter a note').fill('New v29 note');
    await page.getByRole('button', { name: 'Add' }).click();
    await expect(page.getByText('New v29 note')).toBeVisible();
  ...
  test('SC-004: DELETE note with Authorization Bearer', async ({ page }) => {
    ...
    const notes: Note[] = [{ id: 1, title: 'Delete me v29' }];
    ...
    await page.goto('/notes');
    await expect(page.getByText('Delete me v29')).toBeVisible();
    await page.getByRole('button', { name: 'Delete note: Delete me v29' }).click();
    await expect(page.getByText('Delete me v29')).not.toBeVisible();
