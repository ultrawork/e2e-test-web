...
const SEED_NOTES: Note[] = [
  { id: 1, title: 'Note A v33' },
  { id: 2, title: 'Note B v33' },
];

test.describe('Web: /notes Authorization v33', () => {
  test('SC-001: redirects to /login when no auth token', async ({ page }) => {
    ...
    await page.screenshot({ path: 'screenshots/SC-001-redirect-no-token-v33.png' });
  });
  ...
  test('SC-002: GET /api/notes with Bearer token renders notes list', async ({ page }) => {
    ...
    await expect(page.getByText('Note A v33')).toBeVisible();
    await expect(page.getByText('Note B v33')).toBeVisible();
    await page.screenshot({ path: 'screenshots/SC-002-notes-list-v33.png' });
  });

  test('SC-003: POST /api/notes stateful mock with Authorization Bearer', async ({ page }) => {
    await page.addInitScript((t) => localStorage.setItem('token', t), TOKEN);

    const notes: Note[] = [{ id: 1, title: 'Existing v33' }];
    ...
    await page.goto('/notes');
    await page.getByPlaceholder('Enter a note').fill('New v33 note');
    await page.getByRole('button', { name: 'Add' }).click();
    await expect(page.getByText('New v33 note')).toBeVisible();
    await page.screenshot({ path: 'screenshots/SC-003-post-note-v33.png' });
  });

  test('SC-004: DELETE note with Authorization Bearer', async ({ page }) => {
    await page.addInitScript((t) => localStorage.setItem('token', t), TOKEN);

    const notes: Note[] = [{ id: 1, title: 'Delete me v33' }];
    ...
    await page.goto('/notes');
    await expect(page.getByText('Delete me v33')).toBeVisible();
    ...
    await page.screenshot({ path: 'screenshots/SC-004-delete-note-v33.png' });
  });
  ...
  test('SC-005: outgoing requests include Authorization: Bearer token', async ({ page }) => {
    ...
    await page.screenshot({ path: 'screenshots/SC-005-auth-header-v33.png' });
  });
  ...
  test('SC-006: 401 response clears token and redirects to /login', async ({ page }) => {
    ...
    await page.screenshot({ path: 'screenshots/SC-006-401-redirect-v33.png' });
  });
});
